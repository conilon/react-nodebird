...
import { UPLOAD_IMAGE_LIST, ADD_PORTFOLIO_REQUEST, LOAD_PORTFOLIO_REQUEST } from '../../../reducers/portfolio';
...
const Modification = () => {
    ...
    const { fileLists, addedPortfolio, singlePortfolio } = useSelector((state) => state.portfolio);

    console.log('singlePortfolio: ', singlePortfolio);

    const [fileList, setFileList] = useState(singlePortfolio.PortfolioImages);
    const [company, setCompany] = useState(singlePortfolio.company);
    const [website, setWebsite] = useState(singlePortfolio.website);
    const [content, setContent] = useState(singlePortfolio.content);
    const [visible, setVisible] = useState(singlePortfolio.visible);
    const dispatch = useDispatch();

    useEffect(() => {
        if (addedPortfolio.length) {
            alert('포트폴리오가 정상적으로 수정되었습니다.');
...
Modification.getInitialProps = async (context) => {
